...
def registration(request):
    if request.method == 'POST':
        ...
        if person.objects.filter(email=email).exists():
            ...
            person.objects.create(name=name, email=email, contact=contact,
                                 password=password)
...
def login_form(request):
    if request.method == 'POST':
        ...
        if person.objects.filter(contact=contact).exists():
            obj = person.objects.get(contact=contact)
...
def update_form_data(request):
    if request.method == 'POST':
        ...
        email = request.POST['email']
        contact = request.POST['contact']
        person.objects.filter(id=uid).update(name=name, email=email,
                                           contact=contact
                                           )
        return redirect('/table/')
...
def delete(request, pk):
    use = person.objects.filter(id=pk).delete()
    return redirect('/table/')